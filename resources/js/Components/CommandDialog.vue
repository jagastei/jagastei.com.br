...
whenever(arrowup, () => {
    ...
    if(selected.value.startsWith('card-')) {
        const index = result.value.cards.findIndex(card => card.id === selected.value!.split('-')[1]);
        ...
        const index = result.value.accounts.findIndex(account => account.id === selected.value!.split('-')[1]);
...
<template>
    <CommandDialog :open="open" @update:open="emit('update:open', $event)" class="max-w-[450px] border-none">
        ...
        <CommandList>
            ...
            <CommandSeparator v-if="result.accounts.length > 0 && result.cards.length > 0" />

            <CommandGroup v-if="result.cards.length > 0" :heading="$t('Cards')">
                <CommandItem v-for="card in result.cards" :key="card.id" :value="card.id" @select="() => {
