        ...
        <DropdownMenu>
            <DropdownMenuTrigger as-child>
                <Button variant="ghost" size="sm" class="-ml-3 h-8 data-[state=open]:bg-accent">
                    <span>{{ title }}</span>
                    <Icon icon="radix-icons:arrow-up" v-if="column.getIsSorted() === 'desc'" class="ml-2 h-4 w-4" />
                    <Icon icon="radix-icons:arrow-down" v-else-if="column.getIsSorted() === 'asc'" class="ml-2 h-4 w-4" />
                    <Icon icon="radix-icons:caret-sort" v-else class="ml-2 h-4 w-4" />
                </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start">
                <DropdownMenuItem @click="column.toggleSorting(false)">
                    <Icon icon="radix-icons:arrow-up" class="mr-2 h-3.5 w-3.5 text-muted-foreground/70" />
                    Asc
                </DropdownMenuItem>
